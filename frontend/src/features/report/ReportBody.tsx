import type { CommissioningReportDTO, ReportRobotDTO, ReportTaskDTO } from './types'
import { reportDateTime } from './format'

// commissioning-report 5.4 (§3.8, D8/D11/D-R1) — o corpo hierárquico: projeto →
// célula → robô, cada nível com barra de progresso PONDERADO rotulada (rótulo do
// payload, D-R9), o robô com sua Aplicação, e a tabela de tarefas com o histórico
// ABAIXO de cada tarefa. Tarefa SEM histórico não imprime bloco vazio (§3.8).
// Níveis vazios renderizam com barra 0% e sem estourar (§2.9/§1.4). Nada é
...
function Bar({ value, label }: { value: number; label: string }) {
  return (
    <div className="rpt-bar flex items-center gap-2">
      <div className="h-1.5 w-40 overflow-hidden rounded-full bg-bg-sunken" role="progressbar" aria-valuenow={value} aria-valuemin={0} aria-valuemax={100}>
...
function TaskRows({ task, labels: L }: { task: ReportTaskDTO; labels: Labels }) {
  return (
    <>
      <tr className="rpt-task border-t align-top">
